...
class TecNets(MetaLearner):
    ...
    def meta_train(self, task_loader, epoch, writer=None, test=False):
        device = self.device
    ...
    def meta_test(self, task_loader, writer=None):
        with torch.no_grad():
            self.emb_net.eval()
            self.ctr_net.eval()
            self.meta_train(self, task_loader=task_loader, epoch=None, writer=writer, test=True)
